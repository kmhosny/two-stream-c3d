...
def read_file_ids(filename):
    ...
    f.close()
    print("Found %i files belonging to %i classes" % (len(IDs), len(set(labels.values()))))
    return IDs, labels

def split_train_validation(IDs, labels):
    train_ids, validation_ids, _, _ = train_test_split(IDs, list(labels.keys()),
        test_size=0.20, random_state=len(set(labels.values()))
    )
...
def init_generators():
    ...
    train_ids, train_labels, validation_ids, validation_labels = split_train_validation(ids, labels)
...
def main():
    ...
    filepath="./models/c3d_1M_UCF_weights.h5"
    checkpoint = ModelCheckpoint(filepath, monitor=["val_acc"], verbose=1, mode='max')
    callbacks_list = [checkpoint]

    history = model.fit_generator(train_generator, validation_data=validation_generator, epochs=NUM_EPOCHS, workers=2,
        use_multiprocessing=True, shuffle=True, callbacks=callbacks_list)
