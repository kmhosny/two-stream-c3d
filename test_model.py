...
h5model = 'one_network_scratch-l1-498-0.92.h5'
...
def diagnose(data, verbose=True, label='input', plots=False, backend='tf'):
    ...
    if data.ndim > 2:
            ...
            for i in range_this_dim:
                ...
                print("[Info] {}, dim:{} {}-th slice: "
                      "(min, max, mean, std)=({}, {}, {}, {})".format(
                          label, d, i, np.min(sliced), np.max(sliced),
                          np.mean(sliced), np.std(sliced)))
        if plots:
            # assume (l, h, w, c)-shaped input
            if data.ndim != 4:
                print(
                    "[Error] data (shape={}) is not 4-dim. Check data".format(
                        data.shape))
                return
            l, h, w, c = data.shape
            if l >= min_num_spatial_axes or \
                ...
                print(
                    "[Error] data (shape={}) does not look like in (l,h,w,c) "
        ...
        print("[Info] {} (min, max, mean, std)=({}, {}, {}, {})".format(
            label, np.min(data), np.max(data), np.mean(data), np.std(data)))
